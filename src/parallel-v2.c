...
int solve(int line, int column, int matrix[9][9],int layer){
...
until find a solution to the problem, if there is one! */
    ...
    /* This position is already filled, there is nothing to be done. */
        ...
        completed = solve(new_line,new_column,matrix,layer);
        if (completed == 1){
            /* table completed */
            return 1;
        }
    ...
    /* We have to complete this position with one of the possible choices */
        ...
        else if ((sum > 1) && (layer < MAX_LAYERS)){
            new_line = line;
            new_column = column;
            int completed = new_position(&new_line,&new_column), flag = 0;
#           pragma omp parallel default(none) \
            shared(matrix,new_line,new_column,line,column,flag,possible_choices,layer)
            {
#               pragma omp single nowait
                {
                    for (int i=0;i<9;i++){
                        if (possible_choices[i] == 1){
                            int i_copy = i;
#                           pragma omp task firstprivate(i_copy) 
                            {
                                int copy[9][9];
                                memcpy(copy,matrix,9*9*sizeof(int));
                                copy[line][column] = i_copy + 1;
                                int completed = 0;
                                if (flag == 0){
                                    completed = solve(new_line,new_column,copy,layer+1);
                                    if (completed == 1)
                                        flag = 1;
                                }
                            }
